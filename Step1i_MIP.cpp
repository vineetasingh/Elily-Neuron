...
{
	string stringden = format("C:\\CCHMC\\NGN2\\Old STEP1i\\Segmented_Orignal\\%s_dend_processed.png", imname.c_str());
	int co = 0;
	int gauKsize = 11;
	Mat org = image.clone();
	...
	for (int d = 0; d < contours.size(); d++)
	{
		...
		if (chaid > 2500)
		{
			...
			drawContours(org, contours, d, Scalar(0, 255,125), 2, 8, vector<Vec4i>(), 0, Point());
		}
	}
	imwrite(stringden, org);
...
void findsoma(Mat im, Mat imr, vector<vector<Point>> blue_contours, vector<vector<Point>> intersection_contours, ofstream &myfile, string fstr)
{
	...
	else
	{
		...
		string out_string = format("C:\\CCHMC\\NGN2\\Old STEP1i\\Segmented_Orignal\\%s_blue_processed.png", fstr.c_str());
...
int main(int argc, char** argv)
{
	...
	while (std::getline(file, fstr))
	{
		...
		for (int n = 1; n <= 40; n++)//differnt z scales
		{
			...
			if (n < 10)
				name = format("C:\\CCHMC\\NGN2\\Old STEP1i\\%s\\z%d_3layers_original.tif", fstr.c_str(), n);
			...
			string result = format("C:\\CCHMC\\NGN2\\Old STEP1i\\Segmented_Orignal\\%s_processed.png", fstr.c_str(), fstr.c_str());
...
POP:
		...
		//	imshow(" mip",result);
		string stringb = format("C:\\CCHMC\\NGN2\\Old STEP1i\\Segmented_Orignal\\%s_blue_original.png", fstr.c_str());
		string stringg = format("C:\\CCHMC\\NGN2\\Old STEP1i\\Segmented_Orignal\\%s_green_original.png", fstr.c_str());
		string stringr = format("C:\\CCHMC\\NGN2\\Old STEP1i\\Segmented_Orignal\\%s_red_original.png", fstr.c_str());
